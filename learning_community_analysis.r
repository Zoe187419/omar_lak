...
#22 in the number of vars when using dummy variables in the GLM below 
beta.matrix = matrix(NA, nrow = N, ncol = 24) 
...
fm.1.1 = fullmatch(
    in_treatment ~ ppty.score, data = temp,
...
variable.balance  = xBalance(
    ...
    data = na.omit(temp), 
    report = c("adj.mean.diffs", "p.values","chisquare.test")
)	
print(variable.balance)

pre_gpa.balance  = xBalance(
    in_treatment ~ pre_gpa,
    strata = list(
        unstratified_comparison = NULL, 
        full_matching_1=~fm.1.1,
        pair_matching_2=~pair.match
